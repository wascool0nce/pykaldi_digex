    ...
    def recognize(self, wav=None):
        ...
        with SequentialMatrixReader(feats_rspec) as feats_reader, \
            ...
            for (fkey, feats), (ikey, ivectors) in zip(feats_reader, ivectors_reader):
                ...
                with open(transcriptions, 'a', encoding='utf-8') as f:
                    print("fkey", fkey)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--iconf', metavar='CONF',
                        help='Путь к .CONF конфигурационному файлу векторного экстрактора')
    ...
    recognizer = Recognizer(args.scp, args.model, args.graph, args.words, args.conf, args.iconf, args.spk2utt,
                            args.output, args.printed, args.log)
