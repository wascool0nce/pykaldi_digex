...
def start_pipeline(wav):
    ...
    try:
        ...
        json_name = str("/app/output/" + wav_stem + ".json")
        make_subtitles_data(segments, transcriptions, utt2spk, json_name)
        # make_ass(wav_name, segments, transcriptions, utt2spk, ass, json_name)
...
if __name__ == '__main__':
    ...
    parser.add_argument('wav', metavar='WAV', nargs='?', default='/app/wav_audio/', help='Путь к .WAV файлам аудио')
    parser.add_argument('output', metavar='OUT', nargs='?', default='/app/output/',
                        help='Путь к директории с результатами распознавания')
    ...
    WAV_DIR = Path(args.wav) or Path('/app/wav_audio/')
    OUTPUT_DIR = Path(args.output) or Path('archive/')
    REC_MODEL = args.rec_model or '/app/models/model/final.mdl'
    REC_GRAPH = args.rec_graph or '/app/models/model/HCLG.fst'
    REC_WORDS = args.rec_words or '/app/models/model/words.txt'
    REC_CONF = args.rec_conf or '/app/models/model/conf/mfcc.conf'
    REC_ICONF = args.rec_iconf or '/app/models/model/conf/ivector_extractor.conf'
    SEGM_MODEL = args.segm_model or '/app/models/model/final.raw'
    SEGM_CONF = args.segm_conf or '/app/models/model/conf/mfcc_hires.conf'
    SEGM_POST = args.segm_post or '/app/models/model/conf/post_output.vec'
